...
def attacked_embedding(updated_data, one_node_selected_nodes, graph_model, dataset_embeddings):
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    updated_data = updated_data.to(device)  # Move data to the appropriate device
...
#________________________________________________________________________
    ...
    class GraphModel(torch.nn.Module):
        def __init__(self, graph_model):
            num_classes = int(updated_data.y.max().item()) + 1  
            super().__init__()

            if graph_model == 'gcn':
                self.conv = GCNConv(updated_data.num_features, 300).to(device)
            elif graph_model == 'gat':
                self.conv = GATConv(updated_data.num_features, 300).to(device)
            elif graph_model == 'sage':
                self.conv = SAGEConv(updated_data.num_features, 300).to(device)
            elif graph_model == 'graphpatcher':
                self.conv = GraphPatcherLayer(updated_data.num_features, 300).to(device)
            elif graph_model == 'gat2':
                self.conv = GATv2Conv(updated_data.num_features, 300, heads=1).to(device)

            else:
                raise ValueError("Invalid graph_model. Choose from 'gcn', 'gat2', 'sage', 'graphcl', or 'graphpatcher'.")
...
#________________________________________________________________________
    ...
    attacked_model = GraphModel(graph_model).to(device)

    criterion = torch.nn.CrossEntropyLoss().to(device)
    optimizer = torch.optim.Adam(attacked_model.parameters(), lr=0.02)
    ...
    for epoch in range(200):
        ...
        optimizer.zero_grad()
        h, z = attacked_model(updated_data.x, updated_data.edge_index)
        loss = criterion(z, updated_data.y.to(device))
        acc = accuracy(z.argmax(dim=1), updated_data.y.to(device))
        loss.backward()
        optimizer.step()
    ...
    #print("- Attacked Embedding is Generated!")
    #print("____________________________________________")
    ...
    mask = [(edge[0].item() not in selected_nodes_set) and (edge[1].item() not in selected_nodes_set) for edge in updated_data.edge_index.t()]
    mask = torch.tensor(mask, dtype=torch.bool, device=device)
    isolated_edge_index = updated_data.edge_index[:, mask]

    attacked_model.eval()
    with torch.no_grad():
        h, z = attacked_model(updated_data.x, isolated_edge_index)

    one_node_selected_node_embeddings = h[one_node_selected_nodes]


    #print("- Attacked Query Embedding is Generated!")
    #print("____________________________________________")
...
def attacked_return(selected_nodes, selected_node_embeddings_attacked, attacked_dataset_embeddings, top_k_indice_at_20, top_k_indice_at_100, top_k_indice_at_500, top_k_indice_at_1000, top_k_indice_at_4000):
...
def per_node_attacked_return(data_num_nodes, attacked_dataset_embeddings, one_node_selected_node_embedding, node_id, attacked_at_num_node_dict):
    ...
    #top_k = 100
    #attacked_top_k_indice_at_100_per_node = similarity_1D(top_k, one_node_selected_node_embedding, attacked_dataset_embeddings)
    #attacked_at_100_dict[node_id] = attacked_top_k_indice_at_100_per_node

    #top_k = 500
    #attacked_top_k_indice_at_500_per_node = similarity_1D(top_k, one_node_selected_node_embedding, attacked_dataset_embeddings)
    #attacked_at_500_dict[node_id] = attacked_top_k_indice_at_500_per_node

    #top_k = 1000
    #attacked_top_k_indice_at_1000_per_node = similarity_1D(top_k, one_node_selected_node_embedding, attacked_dataset_embeddings)
    #attacked_at_1000_dict[node_id] = attacked_top_k_indice_at_1000_per_node
...
def per_node_dictionary_return(selected_nodes, top_k_indice_at_20, top_k_indice_at_100, top_k_indice_at_500, top_k_indice_at_1000, top_k_indice_at_4000, attacked_at_num_node_dict, data_num_nodes):
    ...
    #retrieval_position_after_attack_20 = get_avg_position(top_k_indice_at_20, attacked_at_num_node_dict)
    #retrieval_position_after_attack_100 = get_avg_position(top_k_indice_at_100, attacked_at_num_node_dict)
    #retrieval_position_after_attack_500 = get_avg_position(top_k_indice_at_500, attacked_at_num_node_dict)
    #retrieval_position_after_attack_1000 = get_avg_position(top_k_indice_at_1000, attacked_at_num_node_dict)
    #retrieval_position_after_attack_4000 = 0
...
def train_edge_removal_model(edge_performance, data, selected_nodes, in_channels=6, hidden_dim=16, out_channels=8, lr=0.01, epochs=100):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    edge_index = data.edge_index.to(device)  # Ensure edge_index is on the correct device
    ...
    for node, edges in edge_performance.items():
        for edge, values in edges.items():
            edge_list.append([
                node, edge, values[0], values[1], values[2][0],  # position_change
                values[2][1], values[2][2], values[2][3], values[2][4], values[2][5]  # avg_promoted, avg_demoted, ...
            ])
    ...
    # Convert selected_nodes to a list (if it's a tensor)
    selected_nodes_list = selected_nodes.tolist() if isinstance(selected_nodes, torch.Tensor) else selected_nodes

    # Remove data for selected nodes from training set
    train_df = edge_removal_model_df[~edge_removal_model_df["node"].isin(selected_nodes_list)].copy()

    # Convert to tensors and move to device
    features = torch.tensor(train_df[['position_change', 'avg_promoted', 'avg_demoted', 'avg_changed', 'avg_unchanged', 'total_items']].values, dtype=torch.float).to(device)
    labels = torch.tensor(train_df['position_change'].values, dtype=torch.float).to(device)

    # Define Edge Removal Model
    class EdgeRemovalGNN(nn.Module):
        def __init__(self, in_channels, hidden_dim, out_channels):
            super().__init__()
            self.conv1 = pyg_nn.GCNConv(in_channels, hidden_dim)
            self.conv2 = pyg_nn.GCNConv(hidden_dim, out_channels)
            self.fc = nn.Linear(out_channels, 1)  # Predict position change score
        ...
        def forward(self, x, edge_index):
            ...
            x = self.fc(x).squeeze()
            return x  # Score for edge importance

    # Train Model
    edge_removal_model = EdgeRemovalGNN(in_channels, hidden_dim, out_channels).to(device)
    optimizer = torch.optim.Adam(edge_removal_model.parameters(), lr)
    criterion = nn.MSELoss()

    for epoch in range(epochs):
        optimizer.zero_grad()
        predictions = edge_removal_model(features, edge_index)  # Ensure everything is on the same device
    ...
    return edge_removal_model, edge_removal_model_df
